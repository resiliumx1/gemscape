...
const FUEL_LEVELS: Record<string, number> = {}; // Simulated per-vehicle
          ...
          return (
            <div key={v.id} className="aura-glass" style={{ padding: "22px 24px", cursor: "pointer" }} onClick={() => openEdit(v)}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div>
                  <p style={{ fontFamily: "var(--aura-font-heading)", fontSize: 20, color: "var(--aura-text)" }}>{v.name}</p>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                <button onClick={e => { e.stopPropagation(); toggleAvailability(v); }} style={{
                  ...
                  border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.04)", color: "var(--aura-text-muted)",
      ...
      {editing && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(6,14,26,0.6)", backdropFilter: "blur(14px)" }}
          ...
          <div onClick={e => e.stopPropagation()} style={{
            width: 480, maxHeight: "85vh", overflowY: "auto", background: "rgba(12,20,36,0.85)",
            backdropFilter: "blur(22px) saturate(1.4)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 18, padding: "28px",
          }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{
                ...
                border: "none", background: "linear-gradient(135deg, #d4aa44, #c49a38)", color: "#060e1a", cursor: "pointer",
